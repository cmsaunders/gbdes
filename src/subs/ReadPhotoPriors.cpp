    ...
    try {
        ...
        for (YAML::const_iterator i = root.begin(); i != root.end(); ++i) {
            ...
            if (!node.IsMap() || !node["Exposures"])
                throw PhotometryError("Non-map or missing exposures key in prior YAML entry " + priorName);
            ...
            double sigma = 0.001;
            if (node["Sigma"]) sigma = node["Sigma"].as<double>();

            // Location of reference point:
            string deviceName = "S2";
            double xpixel = -1.;
            double ypixel = -1.;
            if (node["Device"]) deviceName = node["Device"].as<string>();
            if (node["Xpixel"]) xpixel = node["Xpixel"].as<double>();
            if (node["Ypixel"]) ypixel = node["Ypixel"].as<double>();
            ...
            if (node["Zeropoint"]) zeropoint = node["Zeropoint"].as<double>();
            if (node["Airmass"]) airmassCoeff = node["Airmass"].as<double>();
            if (node["Color"]) colorCoeff = node["Color"].as<double>();
            if (node["Apcorr"]) apcorrCoeff = node["Apcorr"].as<double>();
            ...
            for (int iExpo = 0; iExpo < exposures.size(); iExpo++) {
                ...
                if (regexMatchAny(exposureRegexes, exposures[iExpo]->name)) {
                    ...
                    if (iInst < 0 || !instruments[iInst]) continue;
            ...
            for (int iExtn = 0; iExtn < extensions.size(); iExtn++) {
                ...
                for (int iMatch = 0; !extnMatch && iMatch < matchingExposures.size(); iMatch++) {
                    if (extn.exposure == matchingExposures[iMatch] &&
                        ...
                        extn.startWcs->toWorld(point.args.xDevice, point.args.yDevice, point.args.xExposure,
                                               point.args.yExposure);
