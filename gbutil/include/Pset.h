...
// Parameters can also be read from command-line arguments using 
// setFromArguments(int argc, char **argv);
// Any argument beginning with "-" is assumed to specify a parameter keyword.
// The next argument (separated by whitespace and/or '=' sign) must be the value.
// Note all arguments have values.
// Routine returns the number of arguments before the first one that starts with
// '-' character, i.e. the number of positional arguments.
// Exception thrown for unknown keyword (such as -help).
...
class Pset {
 public:
  ...
  // Set parameters from command-line arguments.  Returns the number of arguments
  // at the front of set that are *not* parameter specifications (=positional arguments).
  int setFromArguments(int argc, char **argv);
