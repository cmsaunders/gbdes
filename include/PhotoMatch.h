...
public:
    ...
    const SubMap *map;
    Detection() : isClipped(false), fitWeight(1.), expectedTrueChisq(0.), itsMatch(nullptr), map(nullptr) {}
...
public:
    ...
    double getMeanFisher() {
        prepare();
        return meanF;
    }

    // Returned integer is the DOF count
    int accumulateChisq(double &chisq, DVector &beta, SymmetricUpdater &updater, bool reuseAlpha = false);
    // sigmaClip returns true if clipped, and deletes the clipped guy
    // if 2nd arg is true.
    bool sigmaClip(double sigThresh, bool deleteDetection = false);
