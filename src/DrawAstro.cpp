...
int main(int argc, char *argv[]) {
    ...
    Pset parameters;
    {
        ...
        parameters.addMember("outputBase", &outputBase, def, "base of filenames for output FITS files",
                             "astrom");
        parameters.addMember("makeTPV", &makeTPV, def,
                             "Set to fit TPV file and return pixel displacements relative to it", false);
        parameters.addMember("makeFITS", &makeFITS, def, "Draw the FITS images?", true);
        parameters.addMember("decimate", &decimate, def + low, "Decimation factor for output images", 1, 1);
        parameters.addMember("tpvOrder", &tpvOrder, def + low + up, "Order of TPV polynomial", 3, 3, 4);
        parameters.addMember("color", &color, def, "Color assumed for source", 0.61);
        parameters.addMember("nominalPixel", &nominalPixel, def + lowopen, "Reference pixel size (arcsec)",
                             0.264, 0.);
    ...
    try {
        ...
        ofstream *ofs = 0;
        if (makeTPV) ofs = new ofstream(tpvFile.c_str());

        // (inclusive) bounds of pixel numbers
        Bounds<int> ccdBounds(1, 2048, 1, 4096);
        // Size of images to draw
        Bounds<int> drawBounds(1, ccdBounds.getXMax() / decimate, 1, ccdBounds.getYMax() / decimate);
        ...
        auto mapNames = findMatches("^" + instrumentName + "/[NS][[:digit:]]*$", pmc.allMapNames());
        ...
        for (auto device : deviceNames) {
            ...
            Bounds<int> mid(2, drawBounds.getXMax() - 1, 2, drawBounds.getYMax() - 1);
...
#pragma omp parallel for
                ...
                for (int iy = mid.getYMin(); iy <= mid.getYMax(); iy++)
                    for (int ix = mid.getXMin(); ix <= mid.getXMax(); ix++) {
                        double a =
                                (xout(ix + 1, iy) - xout(ix - 1, iy)) *
                                        (yout(ix, iy + 1) - yout(ix, iy - 1)) -
                                (xout(ix, iy + 1) - xout(ix, iy - 1)) * (yout(ix + 1, iy) - yout(ix - 1, iy));
                        if (roundto > 0) a = floor(a * rescale / roundto + 0.5) * roundto;
                        area(ix, iy) = a;
                    }
            }  // End if (makeFITS)
...
#pragma omp parallel for
                    ...
                    for (int iy = mid.getYMin(); iy <= mid.getYMax(); iy++)
                        for (int ix = mid.getXMin(); ix <= mid.getXMax(); ix++) {
                            ...
                            if (roundto > 0) d = floor(d / roundto + 0.5) * roundto;
                            dx(ix, iy) = d;
