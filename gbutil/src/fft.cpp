...
#pragma omp critical (fftw)
  {
     t_array = (DComplex*) fftw_malloc(sizeof(DComplex)*N*(N/2+1));
  }
...
#pragma omp critical (fftw)
  {
    t_array = (double*) fftw_malloc(sizeof(double)*N*N);
  }
  // Copy data into new array to avoid NaN's, etc., but not bothering
  // with scaling, etc.
  for (int i=0; i<N*N; i++)
